/*
 * A291445 -- reverse layered-frontier DP for n <= 350.
 *
 * Count permutations s_1,...,s_n of [n] such that
 *
 *     Sum_{i=1..j} s_i divides Sum_{i=1..j} s_i^3,  1 <= j <= n.
 *
 * The user-requested extension is a(0)=1.  This is a backward search, but
 * unlike 291445_02.c it is not a
 ...
 * [n] to U.  Put S=Sum_{x in U}x and
 ...
 *
 * The full prefix is automatic because Sum_{x=1..n}x^3=T^2, where
 * T=Sum_{x=1..n}x.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       291445_03.c -o 291445_03
 *
 * Examples:
 *   ./291445_03 --upto 350
 *   ./291445_03 --term 350
...
#error "291445_03.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "291445_03.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b291445_03.txt"
#define LOCK_PATH "b291445_03.txt.lock"
                         ...
                         uint64_t difference, Count addition)
{
    ...
    if (frontier->size + 1U >
        ...
        die("two frontier layers exceed A291445_MEMORY_MIB");
    }
                                  ...
                                  double started)
{
    ...
    fprintf(stderr,
            "291445_03: n=%u j=%u processed=%zu/%zu next=%zu "
...
static SearchResult count_layered(unsigned n)
{
    ...
    const uint64_t root_sum = triangular;
    const uint64_t root_cube_sum = triangular * triangular;
    ...
    (void)frontier_add(&current, full_set(n), root_sum,
...
static void record_term(unsigned n, Count value)
{
    ...
    char temporary[] = "b291445_03.txt.tmp.XXXXXX";
...
static Count obtain_term(unsigned n)
{
    ...
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "291445_03: reusing saved a(%u)\n", n);
        }
        return value;
    }
    if (!quiet) {
        fprintf(stderr,
                "291445_03: n=%u reverse layered DP started "
                "(screen=%u)\n", n, screen_depth);
    }
    ...
    if (!quiet) {
        ...
        count_to_text(value, text);
        fprintf(stderr,
                "291445_03: n=%u answer=%s states=%" PRIu64
...
static void usage(const char *program, FILE *stream)
{
    fprintf(stream,
            ...
            "A291445_MEMORY_MIB sets the two-frontier memory limit "
            "(default %" PRIu64 " MiB).\n",
