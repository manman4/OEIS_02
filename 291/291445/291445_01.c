/*
 * A291445 -- reverse sparse subset DP.
 *
 ...
 * S(U) divides Q(U).  If U is good, its valid orders satisfy
 *
 *     C(U) = Sum C(U-{x}),
 *
 ...
 * is n=0..300.  Any unexpected count overflow stops safely instead of
 * wrapping.  Completed terms are saved atomically in b291445_01.txt.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       291445_01.c -o 291445_01
 *
 * Examples:
 *   ./291445_01 --upto 100
 *   ./291445_01 --term 100
...
#error "291445_01.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "291445_01.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b291445_01.txt"
#define LOCK_PATH "b291445_01.txt.lock"
...
static void memo_report_freeze(Memo *memo)
{
    if (!quiet && !memo->freeze_reported) {
        fprintf(stderr,
                "291445_01: n=%u memo frozen at %zu states, %.1f MiB; "
...
static void report_progress(Memo *memo)
{
    ...
    fprintf(stderr,
            "291445_01: n=%u new_states=%zu total_states=%zu "
...
static void record_term(unsigned n, Count value)
{
    ...
    char temporary[] = "b291445_01.txt.tmp.XXXXXX";
...
static Count obtain_term(unsigned n, Memo *memo)
{
    ...
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "291445_01: reusing saved a(%u)\n", n);
        }
        return value;
    }
    if (!quiet) {
        fprintf(stderr, "291445_01: n=%u reverse sparse DP started\n", n);
    }
    ...
    if (!quiet) {
        ...
        count_to_text(value, text);
        fprintf(stderr,
                "291445_01: n=%u answer=%s new_states=%zu "
