/*
 * A291519 -- compact reverse sparse subset DP for n <= 350.
 *
 ...
 *
 * Differences from 291519_01.c, following the compact design of
 * 291355_02.c:
 *
 ...
 * n=0..350; this is needed because the terms exceed 256 bits above this
 * program's former range.  Any unexpected overflow stops safely instead of
 * wrapping.
 * Completed terms are saved atomically in b291519_02.txt.
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       291519_02.c -o 291519_02
 *
 * Examples:
 *   ./291519_02 --upto 350
 *   ./291519_02 --term 350
...
#error "291519_02.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "291519_02.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b291519_02.txt"
#define LOCK_PATH "b291519_02.txt.lock"
...
static void memo_report_freeze(Memo *memo)
{
    if (!quiet && !memo->freeze_reported) {
        fprintf(stderr,
                "291519_02: n=%u memo frozen at %zu states, %.1f MiB; "
...
static void report_progress(Memo *memo)
{
    ...
    fprintf(stderr,
            "291519_02: n=%u new_states=%zu total_states=%zu "
...
static void record_term(unsigned n, Count value)
{
    ...
    char temporary[] = "b291519_02.txt.tmp.XXXXXX";
...
static Count obtain_term(unsigned n, Memo *memo)
{
    ...
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "291519_02: reusing saved a(%u)\n", n);
        }
        return value;
    }
    if (!quiet) {
        fprintf(stderr,
                "291519_02: n=%u compact reverse DP started "
    ...
    if (!quiet) {
        ...
        count_to_text(value, text);
        fprintf(stderr,
                "291519_02: n=%u answer=%s new_states=%zu "
...
static void check_implementation(unsigned maximum)
{
    ...
    const Bitset keys[3] = {
        full_set_without_one(3U), full_set_without_one(4U),
        full_set_without_one(5U)
    };
    Count values[3] = {
