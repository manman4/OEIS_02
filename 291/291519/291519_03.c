/*
 * A291519 -- reverse layered-frontier DP for n <= 350.
 *
 * Count permutations s_1,...,s_n of [n], with s_n=1 for n>0, such that
 *
 *     Sum_{i=1..j} s_i divides Sum_{i=1..j} s_i^3,  1 <= j <= n.
 *
 * This is a backward search, but unlike 291519_02.c it is not a
 ...
 * {2,...,n} to U.  Put S=Sum_{x in U}x and
 * D=S^3-Sum_{x in U}x^3.  Removing x is legal exactly when
 *
 *     S-x divides D.
 *
 * For d=S-x, the child stores S'=d and D'=D-3*S*x*d.  This follows from
 * S^3-d^3-x^3=3*S*d*x and avoids storing the cube sum separately.
 ...
 * Counts are checked unsigned 384-bit integers.  If the configured
 ...
 *
 * The final prefix is automatic because Sum_{x=1..n}x^3=T^2, where
 * T=Sum_{x=1..n}x.  The prefix {2,...,n} is also automatic because
 * T^2-1=(T-1)(T+1).
 *
 * Build:
 *   clang -O3 -std=c11 -Wall -Wextra -Wpedantic \
 *       291519_03.c -o 291519_03
 *
 * Examples:
 *   ./291519_03 --upto 350
 *   ./291519_03 --term 350
...
#error "291519_03.c requires unsigned __int128"
#endif

#if SIZE_MAX < UINT64_MAX
#error "291519_03.c requires a 64-bit size_t"
#endif
...
#define BFILE_PATH "b291519_03.txt"
#define LOCK_PATH "b291519_03.txt.lock"
...
/* Packed record: key bytes, S, D, then the 384-bit path count. */
typedef struct {
...
static uint64_t record_sum(const Frontier *frontier,
                           const unsigned char *record)
{
    uint64_t value;
    memcpy(&value, record + frontier->sum_offset, sizeof(value));
    return value;
}

static uint64_t record_difference(const Frontier *frontier,
                                  const unsigned char *record)
{
    uint64_t value;
    memcpy(&value, record + frontier->difference_offset, sizeof(value));
    return value;
}

static Count record_count(const Frontier *frontier,
                          const unsigned char *record)
{
    Count value;
...
static void record_set_count(const Frontier *frontier,
                             unsigned char *record, Count value)
{
                          ...
                          MemoryTracker *memory)
{
    ...
    frontier->tail_key_bytes =
        frontier->key_bytes % sizeof(uint64_t);
    frontier->sum_offset = frontier->key_bytes;
    frontier->difference_offset =
        frontier->sum_offset + sizeof(uint64_t);
    frontier->count_offset =
        frontier->difference_offset + sizeof(uint64_t);
    frontier->record_bytes =
        frontier->count_offset + sizeof(Count);
...
static bool frontier_add(Frontier *frontier, Bitset key, uint64_t sum,
                         uint64_t difference, Count addition)
{
    size_t identifier;
    if (frontier_find(frontier, key, &identifier)) {
        unsigned char *record = frontier_record(frontier, identifier);
        if (record_sum(frontier, record) != sum ||
            record_difference(frontier, record) != difference) {
            die("internal subset-invariant inconsistency");
        }
        Count value = record_count(frontier, record);
    ...
    if (frontier->size + 1U >
        ...
        die("two frontier layers exceed A291519_MEMORY_MIB");
    }
    ...
    memcpy(record + frontier->sum_offset, &sum, sizeof(sum));
    memcpy(record + frontier->difference_offset,
...
static bool has_continuation(unsigned cardinality, Bitset remaining,
                             uint64_t sum, uint64_t difference,
                             unsigned depth)
{
    ...
    for (unsigned word = 0U; word < MAX_KEY_WORDS; ++word) {
        ...
        while (bits != 0U) {
            ...
            if (difference % child_sum == 0U) {
                ...
                const uint64_t removed_cross =
                    UINT64_C(3) * sum * value * child_sum;
                if (difference < removed_cross) {
                    die("internal continuation invariant failure");
                }
                if (has_continuation(
                        cardinality - 1U,
                        bitset_without(remaining, value), child_sum,
                        difference - removed_cross, depth - 1U)) {
                    return true;
                }
            }
            bits ^= bit;
                                  ...
                                  double started)
{
    ...
    fprintf(stderr,
            "291519_03: n=%u j=%u processed=%zu/%zu next=%zu "
...
static SearchResult count_layered(unsigned n)
{
    ...
    const uint64_t root_sum = triangular - 1U;
    const uint64_t root_cube_sum = triangular * triangular - 1U;
    if (root_cube_sum % root_sum != 0U) {
        return result;
    }
    const uint64_t root_difference =
        root_sum * root_sum * root_sum - root_cube_sum;
    ...
    (void)frontier_add(&current, full_set_without_one(n), root_sum,
                       root_difference, count_from_u64(1U));
    result.states = 1U;
    result.maximum_frontier = 1U;
    const double search_started = monotonic_seconds();

    unsigned cardinality = n - 1U;
    while (cardinality > 1U && current.size != 0U) {
        ...
        for (size_t identifier = 0U; identifier < current.size;
            ...
            const uint64_t sum = record_sum(&current, record);
            const uint64_t difference =
                record_difference(&current, record);
            const Count ways = record_count(&current, record);
                ...
                while (bits != 0U) {
                    ...
                    if (difference % child_sum == 0U) {
                        const uint64_t removed_cross =
                            UINT64_C(3) * sum * value * child_sum;
                        if (difference < removed_cross) {
                            die("internal child invariant failure");
                        }
                        const uint64_t child_difference =
                            difference - removed_cross;
                        const Bitset child =
                            bitset_without(remaining, value);
                        if (screen_depth != 0U &&
                            !has_continuation(
                                cardinality - 1U, child, child_sum,
                                child_difference, screen_depth)) {
                            add_u64_saturating(&layer_pruned, 1U);
                        } else {
                            const bool merged = frontier_add(
                                &next, child, child_sum,
                                child_difference, ways);
...
static void record_term(unsigned n, Count value)
{
    ...
    char temporary[] = "b291519_03.txt.tmp.XXXXXX";
...
static Count obtain_term(unsigned n)
{
    Count value;
    if (load_saved_term(n, &value)) {
        if (!quiet) {
            fprintf(stderr, "291519_03: reusing saved a(%u)\n", n);
        }
        return value;
    }
    if (!quiet) {
        fprintf(stderr,
                "291519_03: n=%u reverse layered DP started "
                "(screen=%u)\n", n, screen_depth);
    }
    ...
    if (!quiet) {
        ...
        count_to_text(value, text);
        fprintf(stderr,
                "291519_03: n=%u answer=%s states=%" PRIu64
...
static void check_implementation(unsigned maximum)
{
    ...
    for (unsigned n = 0U; n <= maximum; ++n) {
        ...
        if (!counts_equal(layered.value, forward) ||
            !counts_equal(layered.value, count_from_u64(known_terms[n]))) {
            quiet = saved_quiet;
...
static void usage(const char *program, FILE *stream)
{
    fprintf(stream,
            ...
            "A291519_MEMORY_MIB sets the two-frontier memory limit "
            "(default %" PRIu64 " MiB).\n",
...
int main(int argc, char **argv)
{
    ...
    if (mode == MODE_CHECK) {
        ...
        const Count value = obtain_term(n);
        ...
        for (unsigned index = 0U; index <= n; ++index) {
            const Count value = obtain_term(index);
