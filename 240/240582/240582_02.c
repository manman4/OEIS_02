...
static inline void poly_add_term_mpz(Poly *p, Term t, const mpz_t add) {
    ...
    if (p->size + 1 >= p->cap_table) {
        fprintf(stderr, "Hash table full (size=%zu, cap=%zu). Increase table_pow.\n", p->size, p->cap_table);
        exit(1);
    }
...
static inline void poly_maybe_compact(Poly *p, Poly *scratch) {
    if (p->size == 0) return;
    // If many zero coefficients accumulated, compact to speed up future scans.
    if (p->nonzero_cnt * 2 < p->size) {
        poly_compact(scratch, p);
        Poly tmp = *p;
        *p = *scratch;
        *scratch = tmp;
    }
...
int main(int argc, char **argv) {
    ...
    for (int n = 3; n <= max_n; n++) {
        ...
        for (int k = 1; k < n; k++) {
            ...
            // t0 = -u0 / (k * a_{n-1})
            poly_clear(&t0);
            poly_maybe_compact(&u0, &t0); // use cleared t0 as scratch
            poly_clear(&t0);
            poly_div_var_scale_neg_ui(&t0, &u0, n - 1, (unsigned long)k);
            poly_maybe_compact(&t0, &u0);

            // s += t0 * a_n^k
            poly_mul_pow_into(&s, &t0, n, (uint8_t)k);
            poly_maybe_compact(&s, &u0);
        }
