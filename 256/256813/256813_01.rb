...
def usage!
  ...
  puts <<~USAGE
    ...
    a(n) = number of q-ary words of length n containing the given pattern.
    ...
    Options:
      --gf, --gf-only
               Print only the generating function for words containing
               the given pattern.

    Examples:
      ruby #{program} 2 ababab 35
      ruby #{program} 3 abcabc 35
      ruby #{program} --gf 3 abcabc
...
def multiply(a, b)
  out = Array.new(a.length + b.length - 1, 0)
...
def containing_terms(den, m, count)
...
puts "Name:"
puts "Number of #{word_class(q)} words of length n containing the word #{pattern}."
puts
puts "Formula:"
puts "G.f.: #{gf}."
